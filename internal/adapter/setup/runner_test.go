		...
		t.Run(
			...
			func(t *testing.T) {
				...
				command := process.Command{
					Name:         tc.command,
					Args:         []string{"run", "--model", "chosen"},
					Stdin:        strings.NewReader("private"),
					Stdout:       io.Discard,
	...
	for _, tc := range []struct {
		...
		t.Run(
			...
			func(t *testing.T) {
				...
				m := NewManager(
					runFunc(func(ctx context.Context, c process.Command) (process.Result, error) {
						installs++
...
func TestInstallationCancellationAndStageDeadline(t *testing.T) {
	...
	for _, before := range []bool{true, false} {
		t.Run(
			map[bool]string{true: "before launch", false: "during consent"}[before],
			func(t *testing.T) {
