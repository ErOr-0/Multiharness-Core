...
func TestPlannerBuildsConstrainedCodexCommand(t *testing.T) {
	...
	if !reflect.DeepEqual(captured.schema, planSchema) {
...
func TestPlannerPromptContainsStructuredTaskInput(t *testing.T) {
	input := validTaskInput(t)
	prompt, err := buildPlannerPrompt(input)
	if err != nil {
		t.Fatalf("buildPlannerPrompt() returned an error: %v", err)
	}
	...
	if !strings.Contains(prompt, string(encoded)) {
		t.Fatal("planner prompt does not contain the exact structured task input")
	}
}
