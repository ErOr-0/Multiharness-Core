...
func TestCodexRepairNeverImportsOpenCodeSession(t *testing.T) {
	...
	request := store.RepairRequest{Input: r.Input, Plan: r.Plan, Implementation: r.Implementation, Validation: r.Validation, Review: store.Review{Summary: "fix edge", Findings: []store.ReviewFinding{{Severity: store.FindingSeverityError, Blocking: true, Description: "broken", RequiredAction: "fix"}}}}
	runner := &fakeProcessRunner{run: func(_ context.Context, c process.Command) (process.Result, error) {
		invocation := captureInvocation(t, c)
