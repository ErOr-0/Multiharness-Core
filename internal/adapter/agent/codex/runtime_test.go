...
func TestRuntimeRecovery(t *testing.T) {
	for _, tc := range []struct {
		...
		{name: "compatible primary retained", cache: "0.153.0", primary: "0.153.0", alternate: "0.154.0", want: "/primary/codex"},
		...
		{name: "missing required flag", cache: "0.153.0", primary: "0.153.0", alternate: "0.153.0", missingFlag: true, want: "/app/codex"},
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			original := process.Command{Name: "codex", Args: []string{"exec", "--model", "chosen-model", "--sandbox", "workspace-write", "-"}, Dir: t.TempDir(), Timeout: time.Minute,
				Stdin: strings.NewReader("private task"), Stdout: io.Discard, Stderr: io.Discard, EnvOverrides: map[string]string{"CUSTOM": "value"}, EnvUnset: []string{"REMOVE"}}
...
func TestRuntimeDiscoveryAndCache(t *testing.T) {
	...
	paths := []string{filepath.Join(root, "codex"), filepath.Join(bin, "codex"), filepath.Join(bin, "unsafe"), filepath.Join(bin, "alias"), filepath.Join(root, "alias")}
