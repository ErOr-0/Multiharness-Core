		...
		t.Run(role, func(t *testing.T) {
			...
			for range 2 {
				if role == "planning" {
					...
					_, err = a.Review(t.Context(), store.ReviewRequest{Input: r.Input, Plan: r.Plan, Implementation: r.Implementation, Validation: r.Validation})
				}
				if err != nil {
					t.Fatal(err)
				}
...
func TestOpenCodeReadOnlyFailsClosed(t *testing.T) {
	...
	for _, cfg := range []Config{{PermissionPolicy: PermissionAutoApprove}, {ExtraArgs: []string{"--pure=false"}}, {ExtraArgs: []string{"--agent=build"}}} {
	...
	for _, line := range []string{`{"type":"step_finish","sessionID":"s","part":{"type":"step-finish"}}`, `{"type":"text","sessionID":"s","part":{"type":"text","text":"not json"}}`, `{"type":"error","error":{"code":"insufficient_quota"}}`} {
