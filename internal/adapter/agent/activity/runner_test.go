		...
		t.Run(
			...
			func(t *testing.T) {
				...
				runner := Runner{
					...
					Runner: runFunc(func(ctx context.Context, command process.Command) (process.Result, error) {
						calls++
				...
				result, err := runner.Run(
					t.Context(),
					process.Command{Name: "fixture", Args: args, Stdout: &captured, EnvOverrides: map[string]string{"SECRET": "unchanged"}},
				)
...
func TestActivityDoesNotProbeRetryOrSwallowCancellation(t *testing.T) {
	for _, args := range [][]string{nil, {"--version"}, {"exec", "--help"}, {"exec", "--json"}} {
		...
		r := Runner{
			Agent:   Codex,
			Observe: func(Event) { t.Fatal("unexpected activity") },
			Runner: runFunc(func(got context.Context, command process.Command) (process.Result, error) {
				calls++
